...
// test también vuelca su evidencia a Documents/Nuvok/llm_test_result.txt
...
Future<Directory> _modelsDir() async {
  ...
  return Directory('${docs.path}/Nuvok/models')..createSync(recursive: true);
}
...
Future<void> _report(String line) async {
  ...
  print(line);
  final docs = await getApplicationDocumentsDirectory();
  File('${docs.path}/Nuvok/llm_test_result.txt')
...
void main() {
  ...
  testWidgets('la IA local genera texto en el runtime de Apple',
      (tester) async {
    final docs = await getApplicationDocumentsDirectory();
    final resultFile = File('${docs.path}/Nuvok/llm_test_result.txt');
    ...
    try {
      ...
      await for (final piece in engine.chat([
        {
          'role': 'system',
          'content': 'Eres el asistente de Nuvok. Responde breve.'
        },
